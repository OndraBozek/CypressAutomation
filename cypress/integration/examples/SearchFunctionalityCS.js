...
describe('Test search functionality', () => {
    ...
    it('Search for term with one result and show detail', () => {
        ...
        cy.wait(1000) //
        cy.get('[data-cy=transaction-list-item-100000435868666]').click()
        cy.get('.mr-auto').scrollIntoView().should('include.text','Únor 2016')
        cy.wait(1000)
        cy.get('.clearable-input--3mLh1HA6 > .g-button > .ico-error').click()
    })

    it('Show suggestion in searchbar', () => {
        cy.get('[data-cy=search-keyword]').click() // Click on searchbar to see suggestions
        cy.get('[data-cy=search-dropdown-item-lastWeek]').should('be.visible')
        cy.wait(1000) // See suggestions
    })
    ...
    it('Search for term with more results', () => {
        ...
        cy.get('.g-button > .ico-error').should('be.visible')
    })
    ...
    it('Print searched data', () => {
        ...
        cy.get('.g-status-info-title').should('include.text','Data k tisku jsou připravena.')
    })
  })